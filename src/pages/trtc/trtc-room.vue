<template name="trtc-room">
  <div id="trtc-room">
    ...
    <div id="board-area">
      <board v-if="showBoard" :roomId="roomId" :userId="userId"></board>
    </div>

    <div id="controll-bar">
      <b-button variant="link" @click="toggleMic" :disabled="!videoState">
      ...
      <b-button variant="link" @click="toggleCamera" :disabled="!videoState">
        <b-iconstack font-scale="1">
      ...
      <b-button variant="link">
        <b-iconstack font-scale="1" @click="toggleVideo">
          <b-icon icon="camera-fill" color="white"></b-icon>
          <b-icon icon="slash" variant="danger" v-if="!videoState"></b-icon>
        </b-iconstack>
      </b-button>

      <b-button variant="link">
        <b-iconstack font-scale="1" @click="toggleBoard">
          <b-icon icon="square-fill" color="white"></b-icon>
          <b-icon icon="slash" variant="danger" v-if="!boardState"></b-icon>
        </b-iconstack>
      </b-button>
...
export default {
  ...
  computed: {
    subStreamWidth() {
      let numVideos = Object.keys(this.remoteVideos).length;
      if (this.localVideo) numVideos++;
      let maxWidth = this.subStreamHeight * 1.78;
      let width = this.videoContainer.width / numVideos;
      return width < maxWidth ? width : maxWidth;
    },

    subStreamHeight() {
      return this.videoContainer.clientHeight;
    },

    showBoard() {
      return (this.boardState && this.roomId && this.userId);
    },
  },
  ...
  methods: {
     ...
     * 本地元素样式设置
     */
    localVideoTypeSetting() {
      let view = this.localVideo;
      // let top = MARGIN_TOP + MARGIN;
      view.style.width = `${this.subStreamWidth}px`;
      view.style.height = `${this.subStreamHeight}px`;
      view.style.backGround = '#000000';
      // view.style.left = '50%';
      // view.style.marginLeft = `${-this.subStreamWidth / 2}px`;
    ...
    startScreenShare(source) {
      ...
      trtcCloud.startScreenCapture(document.getElementById('board-area'));
    },
    ...
    startCameraAndMic() {
      ...
      trtcCloud.startLocalPreview(view);  // 开启本地视频录制并上行推流给房间其它用户
    ...
    initTrtc() {
      ...
      // 监听辅流数据，一般用于屏幕共享 注意：显示辅路画面使用的函数是 startRemoteSubStreamView() 而非 startRemoteView()
      trtcCloud.on(
  ...
  mounted() {
    ...
    // 7. 开启视频
    this.initTrtc();      // 初始化trtc
    this.enableVideo();   // 开启视频
